...
class Ajax_Shoutbox_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			self::SHOUTBOX_ID_BASE, 'Shoutbox',
			array(
				'classname'   => 'Ajax_Shoutbox_Widget',
	...
	static function init() {
		...
		add_filter('shoutbox_message', 'convert_smilies', 100);
		add_filter('shoutbox_message', 'wptexturize', 100);
		add_filter('shoutbox_message', array(__CLASS__, 'custom_texturize'), 100);
		add_filter('shoutbox_message', array(__CLASS__, 'asterisk_formatting'), 100);
		...
		add_filter('shoutbox_message', array(__CLASS__, 'custom_vimeo_embed'), 10, 2);
		add_filter('shoutbox_message', array(__CLASS__, 'custom_vidme_embed'), 10, 2);
		...
		add_filter('shoutbox_message', array(__CLASS__, 'custom_imgur_embed'), 10, 2);
		...
		add_filter('shoutbox_message', array(__CLASS__, 'wp_embed'), 15, 2);
	...
	private static function update_version() {
		...
		if (version_compare($registered_version, self::version, '<')) {
			if (version_compare($registered_version, '2.0.0', '<'))
				self::add_cache_table();
			else if (version_compare($registered_version, '2.2.0', '<')) {
				if ($options = get_option('widget_ajax_shoutbox')) {
					foreach ($options as $key => $one_option) {
						if (is_array($one_option) && !in_array("reverse_order", $one_option)) {
							$options[$key]["reverse_order"] = '';
						}
					}
					update_option('widget_ajax_shoutbox', $options);
				}
			}
	...
	function widget($args, $instance) {
		...
		$output .= "<div id='sb_messages'" . ($instance['reverse_order'] ? " class='reverse-order empty'" : "") . ">";
		$output .= self::content($instance, -1, $instance['reverse_order']);  // load messages into box
		$output .= "</div>\n"; // sb_messages

		if ($instance['reverse_order'])
			$output .= "<script type='text/javascript'>setTimeout(function(){var m=jQuery('#" . $args['widget_id'] . " #sb_messages');if(m.length>0){m.scrollTop(m[0].scrollHeight);}},200);</script>";
		...
		if ($instance['registered_user'] == 'true' && !is_user_logged_in())
			...
			$cookie_name = $args['widget_id'] . '_input_height';
			$output .= "<div class=\"resizable\"" . (isset($_COOKIE[$cookie_name]) ? " style=\"height:{$_COOKIE[$cookie_name]}\"" : "") . "><textarea id='sb_message' class='sb_input' placeholder='" . __('Message', 'shoutbox2') . "' maxlength='$maxmsglen'></textarea></div>";
			...
			$output .= '<span class="confirm"></span>';
			$output .= "<span id='sb_status'></span>";

			$translatedSmiley = translate_smiley(array(":-)"));
			if ($translatedSmiley == "🙂") {
				$smiley = '<span id="sb_showsmiles" title="' . __('Smilies', 'shoutbox2') .'">' . $translatedSmiley . '</span>';
			} else {
		...
		$output .= "<span class=\"warning\" title=\"" . __('Update of chat failed; will try again shortly', 'shoutbox2') . "\"></span>";
		$output .= "<span class=\"spinner\"></span>";
	...
	function form($instance) {
		...
		$reverse_order        = $this->get_field_name('reverse_order');
		...
		echo "<p><label for=\"$title_name\">" . __('Title:') ."<input class=\"widefat\" id=\"" . $this->get_field_id('title') ."\" name=\"$title_name\"  type=\"text\" value=\"" . htmlspecialchars($instance['title'], ENT_QUOTES) . "\" /></label></p>"
				 ...
				 . "<span><input type=\"checkbox\" id=\"" . $this->get_field_id('reverse_order') . "\" name=\"$reverse_order\" value=\"true\"" . ($instance['reverse_order'] == 'true' ? ' checked="checked"' : '') . " /><label for=\"$reverse_order\">" . __('Newest message at bottom.', 'shoutbox2') . "</label></span>"
	...
	static function enqueue_scripts() {
		$suffix = '.min';
		wp_register_script('simple_ajax_shoutbox', plugins_url("/js/ajax_shoutbox$suffix.js", __FILE__), array('jquery', 'jquery-ui-resizable'));

		if (self::js_to_head())
			self::enqueue_js();

		wp_enqueue_style('ajax_shoutbox', plugins_url("css/ajax_shoutbox$suffix.css", __FILE__), array('wp-jquery-ui-dialog'));

		$theme = wp_get_theme();
		if (file_exists(__DIR__ . "/css/" . $theme->get_template() . "$suffix.css"))
			wp_enqueue_style('ajax_shoutbox_theme', plugins_url("css/" . $theme->get_template() . "$suffix.css", __FILE__));
		if ($theme->get_template() != $theme->get_stylesheet() && file_exists(__DIR__ . "/css/" . $theme->get_stylesheet() . "$suffix.css"))
			wp_enqueue_style('ajax_shoutbox_child_theme', plugins_url("css/" . $theme->get_stylesheet() . "$suffix.css", __FILE__));

		include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
		if (is_plugin_active('wpforo/wpforo.php'))
			wp_enqueue_style('ajax_shoutbox_wpforo', plugins_url("css/wpforo$suffix.css", __FILE__));
	...
	private static function content($options, $limit=-1, $reverse=false) {
		$return = '';
		...
		if ($result = $wpdb->get_results("SELECT *,post_date FROM " . $table_name . " ORDER BY id DESC LIMIT " . $limit)) {
			foreach ($result as $row) {
				$single = self::format_message($row->user_login, $row->message, $row->post_date, $row->id, $row->website, $row->ip, $options);
				if ($reverse)
			...
			$return .= "<div class=\"no-messages\">" . __('No messages.', 'shoutbox2') . "</div>";
		return $return;
	}
	...
	public static function custom_youtube_embed($text, $options = NULL) {
		...
		return preg_replace_callback('#(?<=[\s,]|^)((http|ftp)s?://)?((www\.|m\.)?youtube\.com/watch|youtu\.be/)'.self::url_path.'#i', create_function('$matches', '
			if (preg_match("%(?:youtube\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&(?:amp;)?]v=)|youtu\.be/)([^\"&?/ ]{11})%i", $matches[0], $match))
				return  "<span class=\"youtube-embed-video\" style=\"background-image: url(\'https://i.ytimg.com/vi/$match[1]/hqdefault.jpg\')\"><a href=\"https://www.youtube.com/watch?v=$match[1]\" target=\"_blank\" class=\"play-button\" rel=\"$match[1]\"></a></span>";
			return $matches[0];
		'), $text);
	}

	public static function custom_vimeo_embed($text, $options = NULL) {
		// embed videos from vimeo.com site
		if (is_null($options) || $options['url_clickable'] != 'true')
			return $text;

		return preg_replace_callback('#(?<=[\s,]|^)(?:https?://)?(?:www\.|m\.)?vimeo\.com/(\d+)#i', array(__CLASS__, "custom_vimeo_replace"), $text);
	}

	public static function custom_vimeo_replace($matches) {
		$url = $matches[0];
		$html = self::get_http($url);
		if ($html !== false	&& preg_match('/<meta[^>]+property="og:type"[^>]+content="video"[^>]*>/i', $html)) {
			if (preg_match('/<meta[^>]+property="og:image"[^>]+content="([^"]+)"[^>]*>/i', $html, $imgmatches))
				$img = $imgmatches[1];
			return "<span class=\"vimeo-embed-video\" style=\"background-image: url('$imgmatches[1]')\"><a href=\"$matches[0]\" target=\"_blank\" class=\"play-button\" rel=\"$matches[1]\"></a></span>";
		}
		return $matches[0];
	}

	public static function custom_vidme_embed($text, $options = NULL) {
		// embed videos from vid.me site
		if (is_null($options) || $options['url_clickable'] != 'true')
			return $text;

		return preg_replace_callback('#(?<=[\s,]|^)(?:https?://)?(?:www\.|m\.)?vid\.me/([\w_\-]+)#i', array(__CLASS__, "custom_vidme_replace"), $text);
	}

	public static function custom_vidme_replace($matches) {
		$url = str_replace("http://", "https://", $matches[0]);
		if (substr($url, 0, 6) == "vid.me")
			$url = "https://" . $url;
		$html = self::get_http($url);
		if ($html !== false	&& preg_match('/<meta[^>]+property="og:type"[^>]+content="video.other"[^>]*>/i', $html)) {
			if (preg_match('/<meta[^>]+property="og:image"[^>]+content="([^"]+)"[^>]*>/i', $html, $imgmatches))
				$img = $imgmatches[1];
			return "<span class=\"vidme-embed-video\" style=\"background-image: url('$imgmatches[1]')\"><a href=\"$matches[0]\" target=\"_blank\" class=\"play-button\" rel=\"$matches[1]\"></a></span>";
		}
		return $matches[0];
	}

	public static function wp_embed($text, $options = NULL) {
		// embed for posts coming from wordpress sites
		if (is_null($options) || $options['url_clickable'] != 'true')
			return $text;

		return preg_replace_callback('#(?<=[\s,]|^)((https?://)?([a-z0-9\-\_]+\.)+('.self::tld.')(/[a-z0-9?@%_+:\-\#\.?!&=/();]+))(?=\W|$)#i', array(__CLASS__, "wp_embed_replace"), $text);
	}

	public static function wp_embed_replace($matches) {
		$url = $matches[0];
		if (substr($url, -1) != "/") $url .= "/";
		$url .= "embed/";
		$html = self::get_http($url);
		if ($html !== false) {
			if (preg_match('/<p[^>]+class="wp-embed-heading"[^>]*>\s*<a[^>]+>([^<]+)/', $html, $titmatches)) {
				$title = $titmatches[1];
				if (preg_match('/<div[^>]+class="wp-embed-featured-image["\s][^>]*>\s*<a[^>]+>\s*<img\s[^>]*src="([^"]+)"[^>]*>/', $html, $imgmatches))
					$img = '<img class="featured-image" src="' . $imgmatches[1]. '">';
				if (preg_match('/<div[^>]+class="wp-embed-excerpt["\s][^>]*>\s*<p>\s*([^<]+)</', $html, $excmatches))
					$excerpt = '<span class="excerpt">' . $excmatches[1] . '</span>';
				if (preg_match('/<div[^>]+class="wp-embed-site-title["\s][^>]*>\s*<a\s[^>]*href="([^"]+)"/s', $html, $sitmatches))
					$siteico = '<img class="siteicon" src="' . $sitmatches[1] . '/favicon.ico">';
				if (preg_match('/<div[^>]+class="wp-embed-site-title["\s][^>]*>.*?<span>([^<]+)</s', $html, $sitmatches)) {
					$site = '<span class="site">' . $siteico . $sitmatches[1] . '</span>';
					$siteslug = " " . sanitize_title($sitmatches[1]) . "-embed";
				}
				if (preg_match('/<div[^>]+class="wp-embed-comments["\s][^>]*>.*?<\/span>([^<]+)</s', $html, $commatches))
					$comments = '<span class="comments">' . $commatches[1] . '</span>';
				return "<div class=\"wp-embed$siteslug\"><a href=\"{$matches[0]}\" target=\"_blank\" class=\"title\">$title$img</a>$excerpt$site$comments  </div>";
			}
		}
		return $matches[0];
	}
	...
	public static function general_embed_replace($matches) {
		$url = $matches[0];
		if (preg_match("#^" . get_option('siteurl') . "/(?:[\w\-]+/)*([\w\-]+)(?:/(?:\?[\w\-\=&]*)?(?:\#[\w\-]*)?)?#", $url, $slugmatch)) {
			$args = array(
				'name'        => $slugmatch[1],
				'post_type'   => 'post',
				'post_status' => 'publish',
			...
			if ($my_posts) {
				$title = preg_replace("/\s+\|\s+[\w\.]+$/", "", get_the_title($my_posts[0]->ID));
				if (has_post_thumbnail($my_posts[0]->ID))
					$img = get_the_post_thumbnail($my_posts[0]->ID);
	...
	public static function custom_imgur_embed_replace($matches) {
		if ($matches[1] == "") $matches[1] = "http://";
		$url = "$matches[1]i.$matches[2]$matches[3].jpg";
		return "<a href=\"$url\" target=\"_blank\"><img src=\"$url\" alt=\"\" /></a>";
	}

	public static function custom_imgur_embed($text, $options = NULL) {
		if (is_null($options) || $options['url_clickable'] != 'true')
			return $text;

		return preg_replace_callback('~(?:^|(?<=\s))(https?://)?(imgur\.com/)(\w\w[\w]+)~', array(__CLASS__, "custom_imgur_embed_replace"), $text);
	}
	...
	public static function custom_img_embed($text, $options = NULL) {
		...
		return preg_replace_callback('#([\s,]|^)(((http|ftp)s?://)?'.self::$img_match.'\.(jpg|jpeg|png|gif)(?:[/\?]'.self::img_path.')?)#i', create_function('$matches', 'return "$matches[1]<a href=\"".($matches[3]==""?"http://":"").$matches[2]."\" target=\"_blank\"><img src=\"".($matches[3]==""?"http://":"").$matches[2]."\" alt=\"\"></a>";'), $text);
	}
	...
	public static function reply_link($menu, $message_text, $username, $options) {
		if (is_user_logged_in() || !$options['registered_user'])
