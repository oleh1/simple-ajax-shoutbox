...
* new: confirmation mark displayed shortly after successfully posting a new message
* fix: doesn't show Reply link if only registered users can post messages and user is not logged in
* fix: error occuring on posting the very first message to the chat after installation of the plugin
* new: French localization
* new: css styling for site running theme twentyseventeen
* new: css styling for site using wpForo plugin

= 2.2 =

* new: possibility to revert messages order, i.e. oldest message at the top, newest one in the bottom. This is default for newly created widgets.
* new: resizable input field (including browsers that do not support textarea resize by default); remembers the new size and when the page is loaded again, the new size is used
* new: custom embed for Vimeo hosted videos
* new: custom embed for articles on sites running on WordPress get site-slug css class, so it can be custom styled
* new: plugin images changed from traditional raster files into svg, directly inline in css (IP address icon, video play buttons)
* fix: a link detection to self-site now handles also URLs containing query part and anchor part
* fix: video embeds optimized for shorter code
* fix: improved styling for some default WP templates
* fix: smilies button icon compatible with WordPress 4.5
* fix: typo in text

= 2.1.4 =
* new: custom embed for imgur hosted images
* new: custom embed for vid.me hosted videos
* new: custom embed for articles on sites running on WordPress
* fix: lower priority on texturizing messages, so it cannot not corrupt urls

= 2.1.3 =
* new: finetuning for WordPress standard theme twentysixteen
* fix: posting a message containig an url of the site itself led into infinite loop of fetching site content
* fix: youtube custom embed was not handling urls in format youtube.com/watch?time_continue=*&v=*
* fix: youtube custom embed was not handling urls to m.youtube.com
* fix: img embed now able to handle links that follow with additional path
